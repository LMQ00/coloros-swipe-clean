...
列表里本体条目下会展开出该应用的分身子项：**图标沿用 ColorOS 原生分身图标**（右下角带
序号角标），标题为 `拼多多 · 分身1` / `拼多多 · 分身2`，**本体与每个分身各自独立设置**：
给本体设「必杀」不影响分身，反之亦然。

> 分身序号按 ROM 的 user serial 排名得出（实测与图标角标里的数字一致）；
> 取不到序号时标题退回显示真实 userId（`拼多多 · #998`）。
